...
import {
  ...
  shippedOrderAction,
} from "../../redux/Actions/orderFilterAction";
...
const OrderList = () => {
  ...
  const userOrderList = useSelector(
    (state: any) => state.getUserOrderListReducer
  );
  const orderStatusFilter = useSelector(
    (state: any) => state.orderStatusFilter
  );
        ...
        <div className="container mx-auto px-5 py-12">
          ...
          <div className="text-right my-3">
            <select
              ...
              onChange={(e: any) => filterHandler(e)}
              name="orderList_Filtering"
              id="orderList_Filtering"
              className="bg-white text-gray-500 font-bold p-3 border border-emerald-500 focus:outline-none rounded-md"
            >
              <option value="all">All</option>
              <option value={ActionType.PROCESSING}>
                {ActionType.PROCESSING.toUpperCase()}
              </option>
              <option value={ActionType.SHIPPED}>
                {ActionType.SHIPPED.toUpperCase()}
              </option>
              <option value={ActionType.DELIVERED}>
                {ActionType.DELIVERED.toUpperCase()}
              </option>
            </select>
          </div>
          <div>
            {filtered.length > 0 ? (
              filtered.map((orderList: any, key: number) => (
                <UserOrderCard key={key} orderList={orderList} />
              ))
            ) : userOrderList.data.length > 0 ? (
              userOrderList.data.map((orderList: any, key: number) => (
                <UserOrderCard key={key} orderList={orderList} />
              ))
            ) : (
              <h1 className="text-3xl mt-8 text-emerald-500 font-bold">
                No Orders Found
              </h1>
            )}
          </div>
