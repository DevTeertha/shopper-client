...
const LoginForm = () => {
  ...
  const userInfoChangeHandler = (e: React.ChangeEvent<HTMLInputElement>): void => {
    ...
    <div className='container mx-auto px-4'>
      <div className='h-screen flex bg-gray-bg1'>
        <div className='w-full max-w-md m-auto bg-white rounded-lg border border-primaryBorder shadow-default py-10 px-16'>
          ...
          <form id='login_form' onSubmit={(e: React.SyntheticEvent) => loginHandler(e)}>
            <div>
              <label htmlFor='email'>Email</label>
              <input
                onBlur={(e: React.ChangeEvent<HTMLInputElement>) => userInfoChangeHandler(e)}
                name='email'
                type='email'
            ...
            <div className='pt-5'>
              ...
              <input onBlur={(e: React.ChangeEvent<HTMLInputElement>) => userInfoChangeHandler(e)} name='password' type='password' className={`w-full orm-control relative flex-auto min-w-0 block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-emerald-600 focus:outline-none`} id='password' placeholder='Your Password' />
            </div>

            <div className='flex justify-center items-center mt-6'>
              {
                loginDetails.loading ? <button type="button" className="cursor-not-allowed w-100 bg-emerald-700 py-2 px-4 text-sm text-white rounded border border-green focus:outline-none focus:border-green-dark" disabled>
                  ...
                  <button type='submit' className={`w-100 bg-emerald-500 py-2 px-4 text-sm text-white rounded border border-green focus:outline-none focus:border-green-dark`}>
                    Login
                  </button>
              }
            </div>
