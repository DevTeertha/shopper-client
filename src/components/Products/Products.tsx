...
const Products: React.FC = () => {
  ...
  const sortedProductsState = useSelector(
    (state: any) => state.sortedProductState
  );
    ...
    <>
      ...
      <section className="py-14">
        <div className="flex justify-between mb-10">
          ...
          <div>
            <select
              onChange={(e: any) => sortHandler(e)}
              className="px-5 py-2 outline-none border border-emerald-500 rounded-md"
            >
        ...
        <div className="grid justify-center sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {sortedProductsState.length ? (
            searchHandler(sortedProductsState, searched).length > 0 ? (
              searchHandler(sortedProductsState, searched).map(
                (product: IProduct) => (
