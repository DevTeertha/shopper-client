  ...
  switch (action.type) {
    case ActionType.ADD_TO_CART: {
      const itemIndex: any = state.cartItems.findIndex(
        (item: cartItemI) => item._id === action.payload._id
      );
      alert("Product added to cart successfully!");
      if (itemIndex >= 0) {
        state.cartItems[itemIndex].quantity =
          parseInt(state.cartItems[itemIndex].quantity) +
        ...
        totalPrice =
          totalPrice +
          parseInt(action.payload.quantity) *
            parseInt(action.payload.price.split(",").join(""));
    ...
    case ActionType.REMOVE_FROM_CART: {
      ...
      const newState = state.cartItems.filter(
        (list: any) => list._id !== action.payload
      );
